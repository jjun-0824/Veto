...
def blend_teacher_logps(teacher_logps: torch.Tensor,
                        student_logps: torch.Tensor,
                        B: float) -> torch.Tensor:
    """
    teacher_logps' = softmax( log p_t + B * log p_s ) in log-space
    (i.e. re-normalize log p_t + B*log p_s with log-softmax)
    Shapes: [..., V]
    """
    if B is None or B == 0.0:
        return teacher_logps
    mixed = teacher_logps + B * student_logps
...
def get_scheduled_B(
    ...
    B_start, B_end: start / end B values
    schedule: "linear" or "const"
    """
    if schedule == "const" or total_steps <= 0:
        return B_start
    ...
    if schedule == "linear":
        return B_start + (B_end - B_start) * t
    else:
        # add cosine, etc. here if needed
        return B_start

def save_checkpoint(
    accelerator, tokenizer, assistant_model, save_dir: str,
):
    ...
    for src, ref in tqdm(zip(vali_data_src, vali_data_ref),
      ...
      if loss_type == "ce":
        ...
        shift_teacher_logps = teacher_outputs.logits[
            0, inputs.input_ids.shape[1]:].log_softmax(dim=-1).contiguous()
        # RuntimeError: The size of tensor a (151646) must match the size of tensor b (151936) 
...
@click.option("-B_start", "B_start", type=float, default=0.3)
@click.option("-B_end", "B_end", type=float, default=0.0)
@click.option("-B_schedule", "B_schedule", type=str, default="linear")
def main(top_k, top_p, task_type, grad_acc_size, kd_type, distance_metric,
         ...
         B_start, B_end, B_schedule):    # NEW params
  ...
  if enable_stop_token:
    if task_type == "gsm_1k" or task_type == "gsm_100":
      ...
      end_of_string_ls = ["\nuser", "\n\nuser", "\n\n\nuser"]
    elif task_type == "fever_1k" or task_type == "fever_100":
      end_of_string_ls = ["\nuser", "\n\nuser", "\n\n\nuser"]
    else:
  ...
  # uncomment for seq_kd: assistant_model = accelerator.prepare_model(
  ...
  if kd_type != "seq_kd":
    ...
    #### tokenizer fix
    print(f"Original model vocab size: {model.config.vocab_size}") # # 152064
    print(f"Original assistant vocab size: {assistant_model.config.vocab_size}") # # 151936
    model.resize_token_embeddings(len(tokenizer)) 
    assistant_model.resize_token_embeddings(len(tokenizer)) # this also changes the tokenizer's own vocab size
    print(f"New model vocab size: {model.config.vocab_size}") # 151646
    print(f"New assistant vocab size: {assistant_model.config.vocab_size}") # 151646
    # print(model.get_input_embeddings().weight.shape[0]) # 152064
    # print(model.lm_head.weight.shape[0]) # 152064
    print(f"assistant pad: {assistant_model.config.pad_token_id}") # 151643
    assistant_model.config.pad_token_id = tokenizer.pad_token_id
    assistant_model.config.eos_token_id = tokenizer.eos_token_id
    print(f"new assistant pad: {assistant_model.config.pad_token_id}") # 151643
    print(f"model pad: {model.config.pad_token_id}") # None
    model.config.pad_token_id = tokenizer.pad_token_id
    model.config.eos_token_id = tokenizer.eos_token_id
    print(f"new model pad: {model.config.pad_token_id}") # 151643
  ...
  if task_type == "gsm_1k" or task_type == "gsm_100":
    ...
    data = load_dataset(
        "json", data_files="data/math_1k_train.json", field="instances",
        split="train"
    )
    if (kd_type == "supervised_kd" or kd_type == "mixed" or
        kd_type == "mixed_skd" or kd_type == "mixed_train"):
    ...
    train_data_src = [ele for ele in data["instruction"]]
    vali_data = load_dataset(
        "json", data_files="data/math_1k_vali.json", field="instances",
        split="train"
    )
    vali_data_src = [ele for ele in vali_data["instruction"]]
    vali_data_ref = [ele for ele in vali_data["response"]]
    ...
    data = load_dataset(
        "json",
        data_files="data/wizardcoder_evol_1k_train.json",  # path where you saved it
        field="instances",
        split="train",
    )

    if (kd_type in ["supervised_kd", "mixed", "mixed_skd", "mixed_train", "seq_kd"]):
      train_src_ref_dict = {
          src: ref for src, ref in zip(data["instruction"], data["response"])
      }
    ...
    vali_data = load_dataset(
        "json",
        data_files="data/wizardcoder_evol_1k_vali.json",   # prepare the validation set in the same format
        field="instances",
        split="train",
    )
    vali_data_src = [ele for ele in vali_data["instruction"]]
    ...
    # supervised_kd / mixed variants need a reference-answer dict
    if (kd_type == "supervised_kd" or kd_type == "mixed" or
        kd_type == "mixed_skd" or kd_type == "mixed_train" or kd_type == "seq_kd"):
      train_src_ref_dict = {
          src: ref for src, ref in zip(data["instruction"], data["response"])
      }
    ...
    # recommended to keep a validation file (or split off part of train)
    vali_data = load_dataset(
        "json", data_files=f"data/{task_type}_vali.json",
        field="instances", split="train"
    )
    vali_data_src = [ele for ele in vali_data["instruction"]]
    vali_data_ref = [ele for ele in vali_data["response"]]
  ...
  save_inp_path  = f"{task_type}_seed_{seed}_{kd_type}_{distance_metric}_{top_k}_{student_save_name}_jsd_beta_{jsd_beta}_{ckpt_prefix}.input.rank{rank}"
  save_out_path  = f"{task_type}_seed_{seed}_{kd_type}_{distance_metric}_{top_k}_{student_save_name}_jsd_beta_{jsd_beta}_{ckpt_prefix}.output.rank{rank}"

  save_inp_data_file  = open(save_inp_path, "w")
  save_out_data_file  = open(save_out_path, "w")
  ...
  with accelerator.split_between_processes(
    ...
    with tqdm(total=len(list(train_data_src_tot))*n_epoch) as pbar:
          ...
          for prompt in prompts:
            with torch.no_grad():
              if kd_type == "on-policy":
                ...
                inputs = tokenizer.apply_chat_template(
                    ...
                    max_length=inp_length,
                    add_generation_prompt=True,
                ).to("cuda") # or .to(accelerator.device) #
                final_outputs = model.module.generate(
                    **inputs,
                    ...
                    top_p=teacher_top_p,
                    return_dict_in_generate=True,
                    # cache_implementation="dynamic", ### extra
                    # return_legacy_cache=True,
                )
                print("Outside cor rate: ", final_outputs.correction_rate)
                temp_report_dict['correction_rate'] = final_outputs.correction_rate
              ...
              uid = f"{rank}-{cur_epoch_index}-{all_steps}"
              save_inp_data_file.write(uid + "\t" + prompt + "[SEP_WENDA]\n")
              save_out_data_file.write(uid + "\t" + new_str + "[SEP_WENDA]\n")
            ...
            if kd_type == "seq_kd":
              ...
              if (
                ...
                # input, target for kl
                # ######### new teacher ###########
                # fewer student generations is better here
                # teacher_blend_B = 0.3

                teacher_blend_B = get_scheduled_B(
                    step=all_steps,          # global step counter
                    total_steps=max_step,    # total steps computed above
                    B_start=B_start,
                    B_end=B_end,
                    schedule=B_schedule,
                )
                ...
                if accelerator.is_main_process and all_steps % eval_step == 0:
                    print(f"[B schedule] step={all_steps}/{max_step}, B={teacher_blend_B:.4f}")

                target_logps = blend_teacher_logps(shift_teacher_logps, shift_student_logps.detach(), teacher_blend_B)
                loss = loss_funct(shift_student_logps, target_logps)
                temp_report_dict['B_value'] = teacher_blend_B
              else:
      ...
      # flush file buffers
      save_inp_data_file.flush()
      save_out_data_file.flush()

      # save the final checkpoint (with 'final' tag)
      if kd_type in ["skd", "mixed_skd"]:
          final_save = f'{prefix}/{task_type}_seed_{seed}_{kd_type}_{distance_metric}_{top_k}_final_{student_save_name}_jsd_beta_{jsd_beta}_{ckpt_prefix}'
      else:
          final_save = f'{prefix}/{task_type}_seed_{seed}_{kd_type}_{distance_metric}_final_{student_save_name}_jsd_beta_{jsd_beta}_{ckpt_prefix}'

      save_checkpoint(accelerator, tokenizer, assistant_model, final_save)

      # close file handles
      save_inp_data_file.close()
      save_out_data_file.close()
      ...
      if not debug_enable and accelerator.is_main_process:
          try:
              ...
              print(f"[wandb] finish() skipped: {e}")
